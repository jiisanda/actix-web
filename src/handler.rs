...
async fn health_checker_handler() -> impl Responder {
    const MESSAGE: &str = "Building simple CRUD API, using Rust, SQLX, Postgres and Actix-Web";
...
pub async fn note_list_handler(
    ...
    if query_result.is_err() {
        let message = "Something bad happened while fetching all notes";
...
async fn create_note_handler(
    ...
    match query_result {
        Ok(note) => {
            let note_response = json!({"state": "success", "data": serde_json::json!({
                "note": note
            })});
...
async fn get_node_handler(
    ...
    match query_result {
        Ok(note) => {
            let note_response =
                json!({"status": "success", "data": serde_json::json!({"note": note})});
